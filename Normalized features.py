...
def plot_combined_roc(roc_data, method_name):
    ...
    # 修改标题为指定的格式
    plt.title(f'ROC Curves - Handcrafted Radiomics features + Clinical/Lab ({method_name.split("(")[-1].replace(")", "")})', fontsize=14)
    plt.legend(loc="lower right", fontsize=12)
    plt.grid(True, alpha=0.2)
    plt.tight_layout()
    plt.savefig(f"Handcrafted_Radiomics_features_Clinical_Lab_{method_name.split('(')[-1].replace(')', '').replace(' ', '_')}_combined_roc.png", dpi=300)
    plt.close()
    print(f"Saved combined ROC curve: Handcrafted_Radiomics_features_Clinical_Lab_{method_name.split('(')[-1].replace(')', '').replace(' ', '_')}_combined_roc.png")
...
def plot_cv_results(cv_results, method_name):
    ...
    for i, metric in enumerate(metrics, 1):
        ...
        # 修改标题为指定的格式
        plt.title(f"{metric} - Handcrafted Radiomics features + Clinical/Lab ({method_name.split('(')[-1].replace(')', '')})", fontsize=12)
    ...
    plt.savefig(f"Handcrafted_Radiomics_features_Clinical_Lab_{method_short}_boxplot.png", dpi=300)
    plt.close()
    print(f"Saved boxplot: Handcrafted_Radiomics_features_Clinical_Lab_{method_short}_boxplot.png")

if __name__ == "__main__":
    print("Starting program execution...")
    
    # 数据加载
    data_matrix_file = '归一化特征.csv'
    ...
    # 修改评估名称
    cv_results_ttest = cross_validate_classifiers(combined_features, y, combined_features.columns, "Handcrafted Radiomics features + Clinical/Lab (T-Test)")
    plot_cv_results(cv_results_ttest, "Handcrafted Radiomics features + Clinical/Lab (T-Test)")
    ...
    # 修改评估名称
    cv_results_rfe_svm = cross_validate_classifiers(combined_features_rfe, y, combined_features_rfe.columns, "Handcrafted Radiomics features + Clinical/Lab (RFE)")
    plot_cv_results(cv_results_rfe_svm, "Handcrafted Radiomics features + Clinical/Lab (RFE)")
    ...
    # 修改评估名称
    cv_results_lasso = cross_validate_classifiers(combined_features_lasso, y, combined_features_lasso.columns, "Handcrafted Radiomics features + Clinical/Lab (LASSO)")
    plot_cv_results(cv_results_lasso, "Handcrafted Radiomics features + Clinical/Lab (LASSO)")
